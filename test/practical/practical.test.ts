...
describe('practical', () => {
  describe('when given a JoinPractical command', () => {
    ...
    describe('and the practical is in the future and has slots available', () => {
      const history = [
        practicalScheduled(practicalId, faker.lorem.words(), [], 2, faker.date.future(), arbitraryDuration()),
      ]
      const result = practical(history)(command)

      it('returns a MemberSignedUpForPractical event', () => {
        expect(result).toStrictEqual([memberSignedUpForPractical(memberNumber, practicalId)])
      })
    })

    describe('and the practical is in the past', () => {
      const history = [
        practicalScheduled(practicalId, faker.lorem.words(), [], 2, faker.date.past(), arbitraryDuration()),
      ]
      const result = practical(history)(command)
